...
.. module:: GcCreds
   :platform: Linux
   :synopsis: Credentials Manager for GlobalConsole
...
class GcCreds:
    ...
    -handle definitions of credentials
    ...
    def __init__(self, credfile=""):
        ...
        self.gLogging = GcLogging("creds")
        self.gConfig = GcConfig().config
        #if credfile not provided, use default one from config file
        #start TinyDB object
        try:
            if len(credfile) == 0:
                credfile = "{}/{}".format(os.path.dirname(os.path.dirname(os.path.abspath(inspect.getsourcefile(GcCreds)))), self.gConfig['JSON']['credfile'])
            ...
            self.gLogging.critical("credentials definitions cannot be loaded. process terminated..")
    ...
    def searchCredName(self, credname):
        ...
        try:
            ...
            #workaround for cannot search with credential name error
            time.sleep(0.3)
            try:
                return self.credtable.search(MyCred.credname == credname)
            except Exception:
...
if __name__ == '__main__':
    creds = GcCreds()
    #creds.encryptPasswds(_utest=True)
    #creds.purgeCreds()
    #creds.importCsvAsCreds("{}/{}".format(os.path.dirname(os.path.dirname(os.path.abspath(inspect.getsourcefile(GcCreds)))), "creds/creds.csv"))
    # creds.updateCred({'encrypted': 'yes', 'password': 'some_pass'}, 'credname3')
    # creds.removeCred('credname3')
    # creds.exportCredsToCsv(outfile='/home/pl55227/Documents/GlobalConsole/GlobalConsole2/creds/credsExp.csv')
    #creds.decryptPasswdsInFile()
    creds.getCreds(True, False, 'encrypted', 'username')
